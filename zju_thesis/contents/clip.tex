...
\chapter{均匀三角形分割算法}
如上文所述，光滑自由变形需将模型沿节点盒切割，并将非三角形的面片三角化。这一过程很可能产生狭长三角形或者蜕化三角形。过多的此类三角形不能会浪费计算资源，还可能带来很多数值计算方面的问题。

另一方面，沿节点盒切割只是为了保证变形结果是严格的$n$次三角贝赛尔曲面片。若略去这一步骤，算法仍能继续，只是结果会变得不精确。考虑到地光滑自由变形中，作者采用以精度换取效率的策略，用三次的三角贝赛尔曲面拟合n次的精确结果，所以光滑自由变形的结果只是精确结果的近似。所以光滑自由变形中“沿节点盒以保证结果精确”这一步骤就显的可有可无了。

因此，本文尝试提出一种新的三角形分割算法，以替换光滑自由变形中的沿节点盒切割这一步骤。
